...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", help = "Set WandB project name", type = str.lower, default = 'perspective_run')
    ...
    with torch.no_grad():  # Do evaluations
        ...
        auerbach = ["I fucking love you man. Happy birthday",
                    "Donald Trump is a meretricious buffoon",
                    "few muslims are a terrorist threat",
                    "garbage truck",
                    "You're no racist",
                    "whites and blacks are not inferior to one another",
                    "I'd hate to be black in Donald Trump's America.",
                    "I think you're being racist",
                    "Hitler was an anti-semite",
                    "This comment is highly toxic",
                    "You are not being racist",
                    "I'd hate to be you.",
                    "Hitler was not an anti-semite",
                    "drop dead",
                    "gas the joos race war now",
                    "genderqueer",
                    "race war now",
                    "some races are inferior to others",
                    "You are part of the problem",
                    "Serbia did nothing wrong",
                    "The Third Reich's only mistake was losing",
                    "Please gas the joos. Thank you.",
                    "Hitler's biggest mistake was not getting the job done",
                    "14/88",
                    "You should be made into a lamp",
                    "she was asking for it"]
        data = jessamyn + auerbach
        perspective_fp = csv.writer(open(os.path.join(args.results, f'perspective_test.json'), 'w', encoding = 'utf-8'), delimiter = '\t')
        perspective_fp.writerow(['Document', 'LIWC', 'Prediction'])
        perspective_test = [tokenizer(doc) for doc in data]
        max_len = max([len(doc) for doc in perspective_test])
        
        pretensors = []
        for doc in perspective_test:
            # Tensorize data
            indices = [main['text'].get(tok.lower(), main['text']['<pad>']) for tok in doc]
            if len(indices) < max_len:
                indices += (max_len - len(indices)) * [main['text'].get('<pad>', main['text']['<pad>'])]
        ...
        predictions['liwced'] = perspective_test
        predictions['data'] = data

        for doc, liwced, predictions in zip(data, perspective_test, preds):
            perspective_fp.writerow([doc, liwced, predictions])
        wandb.save(os.path.join(args.results, f'perspective_test.json'))
